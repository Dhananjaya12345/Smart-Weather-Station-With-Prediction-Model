...
# Fitting the model
history = weather_NN.fit(weather_inputs.astype(np.float32), weather_outputs.astype(np.float32), batch_size=4, epochs=500, validation_split=0.2, verbose=2)
...
new_data = np.array([[5, 175510, 28.1, 90.2, 30]])  # Replace this with your new data

# Predicting the rain and wind speed
prediction = weather_NN.predict(new_data)
...
day_map_inverse = {v: k for k, v in day_dic.items()}
predicted_day = day_map_inverse[new_data[0][0]]
predicted_time = new_data[0][1]
...
data = {
    ...
    'field3': predicted_day,
    'field4': predicted_time
}
